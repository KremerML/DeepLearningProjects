...
def main():
    ...
    for images, labels in tqdm(loader):
        ...
        logits = clipzs.model_inference(batches)
        probabilities, predictions = logits.detach().topk(1,dim=1)
        predictions = predictions.T
        
        true_pos = predictions.eq(labels.view(1,-1).expand_as(predictions))
        accuracy = true_pos.sum()/len(batches)
        
        top1.update(accuracy, len(batches))
