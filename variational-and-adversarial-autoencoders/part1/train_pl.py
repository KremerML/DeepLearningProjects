...
class VAE(pl.LightningModule):
    ...
    def forward(self, imgs):
        ...
        """
        # ELBO(D) = sum(ELBO(x))
        # L_rec = L_rec(D) = average(L_rec(x))
        # L_reg = L_reg(D) = average(L_reg(x))
        # ELBO(x) = -L_rec(x) - L_reg(x)
        # ELBO(D) = -N * AVERAGE(L_rec(x) + L_reg(x))
        # Batch x H x W x Channel
        ...
        # # 4 bit to one-hot encoding
        imgs = torch.nn.functional.one_hot(imgs, num_classes=16)
        imgs = torch.squeeze(imgs, dim=3)
        imgs = torch.permute(imgs, [0,3,1,2])

        L_rec = torch.nn.functional.cross_entropy(output, imgs.float(), reduction='sum') / imgs.shape[0]
        L_reg = KLD(mean, log_std).mean()

        # elbo = -imgs.shape[0] * (L_rec + L_reg)
        elbo = L_rec + L_reg
    ...
    def sample(self, batch_size):
        ...
        # rand_samples = torch.randint(0,15,size=(batch_size,1,28,28))

        p_z = torch.distributions.Normal(0, 1) 
        z_vals = p_z.icdf(values)
        
        z_batch = torch.cartesian_prod(z_vals, z_vals).to(device)

        output = self.decoder.forward(z_batch)
        ...
        x_samples = imgs.reshape((imgs_shape[0],1,imgs_shape[2],imgs_shape[3])).float()
