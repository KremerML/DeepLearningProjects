...
def train_aae(epoch, model, train_loader,
    ...
    for batch_idx, (x, _) in enumerate(train_loader):
        ...
        ae_loss, logs_ae = model.get_loss_autoencoder(x, recon_x, z_fake, lambda_=lambda_)
        logger_ae.add_values(logs_ae)
        ...
        disc_loss, logs_disc = model.get_loss_discriminator(z_fake.detach())
        logger_disc.add_values(logs_disc)
        disc_loss.backward()
        optimizer_disc.step()
...
def main(args):
    ...
    optimizer_ae = optim.Adam(params=model.parameters(),lr=args.ae_lr, betas = (0.5, 0.999))
    optimizer_disc = optim.SGD(params=model.parameters(), lr=args.ae_lr)
